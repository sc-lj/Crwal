...
class SpiderMain(object):
    ...
    def start_check(self, ips, website_name):
        if ips:
            ...
            msg = 'There Are No Available From [{website_name}] Can Be Used To Check, Please Check!!!!!!!'.format( website_name=website_name)
            logger.error(msg)
            pass
    ...
    def save_ip(self, response):
        ...
        # msg = '[{ip}] can visit the target url [{target_url}], source is [{source}]'.format(ip=_ip,target_url=target_url,source=website_name)
        # logger.info(msg)
